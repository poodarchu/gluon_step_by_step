...
class CtcMetrics(object):
    ...
    @staticmethod
    def _remove_blank(l):
        """ Remove trailing zeros in the list of integers and returns a new list of integers """
        ret = []
    ...
    def _lcs(p, l):
        """
            Calculates the Longest Common Subsequence between p and l (both list of int).
            Returns its length.
        """
        # DP to find LCS
        if len(p) == 0:
            return 0
        P = np.array(list(p)).reshape((1, len(p))) # predict output sequence
        L = np.array(list(l)).reshape((len(l), 1)) # label sequence
        M = np.int32(P==L)
    ...
    def accuracy_lcs(self, label, pred):
        """
            LCS accuracy measure: calculate accuracy of each prediction as LCS/Length.
        """
        hit = 0.
        total = 0.
        batch_size = label.shape[0]
